...
from WebServer.Persistence.Models import UserMessage as DBUserMessage, Report as DBReport, UserLanguage as DBUserLanguage
...
class NLPAnalyzer:
    ...
    async def analyze_and_store(
        ...
        async with self.session_factory() as session:  # type: ignore # type: AsyncSession
            ...
            stmt_insert = insert(DBUserMessage).values(
                user_id=user_id,
                prompt_id=prompt_id,
            ...
            stmt_report = insert(DBReport).values(
                user_id=user_id,
    ...
    async def _get_user_language_id(self, session: AsyncSession, user_id: int, prompt_id: int) -> Optional[int]:
        """
        ...
        stmt_ul = select(DBUserLanguage).where(
            (DBUserLanguage.user_id == user_id) & (DBUserLanguage.language_id == prompt_lang_id)
        ).limit(1)
